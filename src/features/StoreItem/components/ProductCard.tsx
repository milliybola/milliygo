...
import { useCartStore, RestaurantInfo } from '@/store/cartStore'
...
const ProductCard = ({
  ...
  const secureProductImage =
    mainImage?.image && mainImage.image !== 'undefined'
      ? mainImage.image.replace('http://', 'https://')
      : null
  ...
  const handleAdd = () => {
    ...
    console.log('StoreItem AddItem result:', success, 'StoreId:', storeId)

    if (!success) {
      alert('Sizda boshqa restorandan mahsulotlar bor!')
      message.warning({
        content:
          "Savatchangizda boshqa restoranning mahsulotlari bor. Davom etish uchun avval ularni o'chirib tashlang.",
  ...
  return (
    ...
    >
      {/* ── Rasm ── */}
      <div className="relative overflow-hidden bg-[#F9FAFB]" style={{ aspectRatio: '1 / 1' }}>
        {/* Badges */}
        ...
        {secureProductImage ? (
          <img
            src={secureProductImage}
            alt={product.name}
            className="h-full w-full cursor-pointer object-contain p-2 transition-transform duration-500 group-hover:scale-105"
          />
        ) : (
      ...
      <div className="flex flex-1 flex-col justify-between p-3.5">
        <div>
          <div className="flex flex-wrap items-baseline gap-1.5">
            <p className="text-[16px] font-extrabold text-[#0c0c0c]">
              ...
              <span className="ml-0.5 text-[12px] font-bold">so'm</span>
            </p>
            {oldPrice && (
          ...
          <p className="mt-1.5 line-clamp-2 min-h-[36px] text-[13px] font-medium leading-snug text-[#374151]">
            {product.name}
          </p>

          {product.description && (
            <p className="mt-1 inline-block rounded-md bg-[#F3F4F6] px-1.5 py-0.5 text-[11px] font-medium text-[#9CA3AF]">
              {product.description}
            </p>
          )}
        </div>
        ...
        <div className="mt-4">
          {qty === 0 ? (
            <button
              onClick={handleAdd}
              className="flex w-full items-center justify-center gap-1 rounded-[12px] border border-[#E5E7EB] py-2.5 text-[13px] font-bold text-[#0c0c0c] transition-all hover:border-[#0c0c0c] hover:bg-[#F9FAFB] active:scale-[0.98]"
            >
              <span className="text-[16px] leading-none">+</span> Qo'shish
            </button>
          ) : (
            <div className="flex items-center justify-between rounded-[12px] border border-[#E5E7EB] bg-[#F9FAFB] p-1 shadow-inner">
              <button
